...
def test_get_boxers_uses_cache(): 
    """If cached and TTL valid, do not hit database."""
    ring_model.ring.append(sample_boxer1.id)
    ring_model._boxer_cache[sample_boxer1.id] = sample_boxer1
    ring_model._ttl[sample_boxer1.id] = time.time() + 100
...
def test_get_boxers_refreshes_on_expired_ttl():

    """Expired TTL should re-load from DB."""
    ring_model.ring.append(sample_boxer1.id)
    ring_model._boxer_cache[sample_boxer1.id] = mocker.Mock()
    ring_model._ttl[sample_boxer1.id] = time.time() - 1

    mock_get = mocker.patch(
        "boxing.models.ring_model.Boxers.get_boxer_by_id",
        return_value=sample_boxer1
    )
    boxers = ring_model.get_boxers()
...
#     assert boxers[0] == sample_boxer1
def test_cache_populated_on_get_boxers(): 

    """On cache miss, should populate both cache and TTL."""
    ring_model.ring.append(sample_boxer1.id)
    mocker.patch(
        "boxing.models.ring_model.Boxers.get_boxer_by_id",
        return_value=sample_boxer1
...
def test_enter_ring(): 
    """Test entering two boxers into the ring in order."""
    ring_model.enter_ring(sample_boxers[0].id)
    assert ring_model.ring == [sample_boxers[0].id]
...
def test_enter_ring_full(): 
    """Test that entering a third boxer raises when ring is full."""
    ring_model.ring = [1, 2]
    with pytest.raises(ValueError, match="Ring is full"):
        ring_model.enter_ring(3)
...
def test_get_fighting_skill(): 
    """Verify the skill formula for two weight classes."""
    # for Ali: 210 * 12 + (78 / 10)
    expected_1 = sample_boxers[0].weight * 12 + (sample_boxers[0].reach / 10)
    # for Tyson: 220 * 10 + (71 / 10) - 1
    expected_2 = sample_boxers[1].weight * 10 + (sample_boxers[1].reach / 10) - 1
...
#     assert ring_model.ring == []
def test_fight(): 
    """Test a full fight flow, winner determined by mocked skills & randomness."""
    ring_model.ring.extend([b.id for b in sample_boxers])

    mocker.patch(
        "boxing.models.ring_model.RingModel.get_fighting_skill",
        side_effect=[2526.8, 2206.1]
    )
    mocker.patch("boxing.models.ring_model.get_random", return_value=0.42)
    mocker.patch(
        "boxing.models.ring_model.RingModel.get_boxers",
        return_value=sample_boxers
    )
    mock_update = mocker.patch("boxing.models.ring_model.Boxers.update_stats")
...
#         ring_model.fight()
def test_fight_with_empty_ring(): 
    """Fight with <2 boxers should raise error."""
    with pytest.raises(ValueError, match="There must be two boxers to start a fight."):
        ring_model.fight()
...
#     with pytest.raises(ValueError, match="There must be two boxers to start a fight."):
#         ring_model.fight()
def test_fight_with_one_boxer(): 
    """Fight with only one boxer should raise error."""
    ring_model.ring.append(sample_boxer1.id)
    with pytest.raises(ValueError, match="There must be two boxers to start a fight."):
        ring_model.fight()
...
def test_clear_cache(): 
    """Test that clear_cache empties both cache dicts."""
    ring_model._boxer_cache[sample_boxer1.id] = sample_boxer1
    ring_model._ttl[sample_boxer1.id] = time.time() + 100
